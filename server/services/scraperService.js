...
export async function scraperUrl(url) {
  ...
  try {
    ...
    // extact all the SEO-relevant data from the page rendered content
    ...
    console.error("[SRAPER] Playwrite failes:" , error.message);
    if(browser){
        try {
            await browser.close().catch(() => {});
        } catch (error) {
            console.error("[SRAPER] Browser close failed:" , error.message);
        }
    }
  }
}
