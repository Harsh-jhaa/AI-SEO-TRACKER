...
export default function Analyze() {
  ...
  const handleAnalyze = async (submitUrl?: string) => {
    ...
    try {
        // step: 0 Connecting to browser
        setCurrentStep(0);

        const res = await api.post('/api/analysis/analyze', {
          url: targetUrl.startsWith('https://')
            ? targetUrl
            : `https://${targetUrl}`,
        });

        // console.log('1. Backend response',res.data);

        if(!res.data.success) {
            throw new Error(res.data.message);
        }
        const id = res.data.analysisId;
        // console.log("2. Analysis ID",id);
        ...
        pollRef.current = setInterval(async () => {
            ...
            try {
                ...
                if(analysis.status === 'completed'){
                    ...
                    // still preocessing - advance visual steps
